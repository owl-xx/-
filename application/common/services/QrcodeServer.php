...
class QrcodeServer
{
    ...
    protected $_file_name       = './static/qrcode_good/';  // 写入文件路径
    ...
    public function generateImg($file_name) {
        ...
        $result_file_name =  uniqid().'.' . self::WRITE_NAME;  //文件名

        $result_file_path = $file_name.DIRECTORY_SEPARATOR. $result_file_name;
        ...
        try {
            $this->_qr->writeFile($result_file_path);
            $data = [
                'url' => $result_file_path,
